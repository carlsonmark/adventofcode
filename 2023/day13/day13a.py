...
def symm_vert(a: np.ndarray) -> int:
    ...
    for row in range(num_rows-1):
        if (a[row] == a[row + 1]).all():
            is_symm = check_symm_vert(a, row + 1)
            if is_symm:
                return row + 1
    return None

def check_symm_horiz(a: np.ndarray, pos: int) -> bool:
    return check_symm_vert(a.transpose(), pos)

def check_symm_vert(a: np.ndarray, pos: int) -> bool:
    # if pos <= 1 or pos > (a.shape[0] - 2):
    #     return True
    num_rows = a.shape[0]
    ...
    return (first_half == flipped_half).all()
...
# 27837 too low, had h and v score scale swapped
