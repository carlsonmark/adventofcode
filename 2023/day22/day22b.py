...
# Part 1 was most of the way there already, just need to keep track of the
# number of dropped bricks when settling instead of just a flag.
...
def settle_bricks(bricks, puzzle):
    ...
    for brick in sorted(bricks):
        ...
        if not settled:
            ...
            brick.drop(1)
            dropped_set.add(brick.number)
        for cube in brick.cubes:
            puzzle[cube.to_coords()] = brick.number
        if not settled:
            all_settled = False
    # print(dropped_set)
    return all_settled, dropped_set, puzzle
...
def solve(lines: str):
    ...
    while not all_settled:
        all_settled, dropped_set, puzzle = settle_bricks(bricks, puzzle)
        # for brick in bricks:
        #     print(brick)
        # show_puzzle(puzzle)
    ...
    for brick in bricks:
        ...
        all_settled, dropped_set, puzzle = settle_bricks(bricks_copy, puzzle)
        if not all_settled:
            total_dropped += len(dropped_set)
        print(f'removed {brick}, {all_settled=}, {len(dropped_set)=}')
        # show_slices(puzzle)
    print(total_dropped)
    return
