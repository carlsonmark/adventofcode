# Notes
# Have to go back to the start too... and then go back to the end
# Hopefully we can just try to get back to the start from the shortest
# result from part A, then back to the end from the shortest result from that!
# Yeah, that should work, because the elf could just wait at the start position.
...
def moves(m, elf_pos):
    ...
    rows, cols = m.shape
    row, col = elf_pos
    if elf_pos == (rows, cols-1):
        # Special case: First move
        if m[rows - 1, cols - 1] == 0:
            moves_ = {DIR_UP, DIR_NONE}
        else:
            moves_ = {DIR_NONE}
    elif elf_pos == (0, 0):
        # Special case: End
...
def find_optimal_path(m, elf_pos, move_count, best_move_count):
    ...
    too_long = (rows + cols) * 4 - (rows - row) - (cols - col)
...
# Best for each leg of the journey: 305, 284, 316
