# Notes:
# Simulating CPU cycles
# Only two commands? addx and noop
# Keep track of the cycle count
# "addx" takes two cycles, "noop" takes one
# Signal strength is the cycle number multiplied by the X register value
# during the 20th cycle and every 40th cycle after that.
...
while instructions:
    if cycle_count in (20, 60, 100, 140, 180, 220):
        x_values[cycle_count] = registers['X']
    instructions = execute_cycle(registers, instructions)
    cycle_count += 1

print(f'{x_values=}')
sum_ = 0
for cycle, value in x_values.items():
    sum_ += cycle * value
print(f'{sum_=}')
